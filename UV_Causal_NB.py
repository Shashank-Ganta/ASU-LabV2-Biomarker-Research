...
maxDepth = 2
thresholdInput = 1.0
method = "S2_Causal_NB" + str(thresholdInput)[0] + "_" + str(thresholdInput)[2]
...
for threshold in varyingThresholds:
    ...
    def combinedCausalityApproach(indexs, trainTotalData,counter,testData,testIndex):
        ...
        for proteinNameCausal in bioMarkerCausalValues:
            if proteinNameCausal in DisjunctiveEAVG.keys():
                bioMarkerCausalValues[proteinNameCausal].append(DisjunctiveEAVG[proteinNameCausal])
            else:
                bioMarkerCausalValues[proteinNameCausal].append("-")
    ...
    def makeROC():
        ...
        with open('predictProbaScores' + str(method) + str(threshold) + '_50.csv','w') as csvfile:
            rocPrinter = [["Fold","d10_y_test","d10_y_proba","d15_y_test","d15_y_proba","d30_y_test","d30_y_proba","d1_y_test","d1_y_proba","d3_y_test","d3_y_proba","d4_y_test","d4_y_proba"]]
...
def printing():
    ...
    with open("C:\\Users\\shash\\VS Code Programs\\Python\\ResearchWork\\Gastric Cancer Data\\gastricSBATCHFiles\\s2causalScoresUpdated10.csv", 'w') as csvfile:
                ROCwriter = cs.writer(csvfile, delimiter=',',
                    quotechar='|', quoting=cs.QUOTE_MINIMAL,lineterminator = '\n')
                #filewriter.writerows(fieldName)
