...
for threshold in varyingThresholds:
    ...
    def combinedCausalityApproach(indexs, trainTotalData,counter,testData,testIndex):
        ...
        for bioMarker in usableProteins:
            ...
            #binarizes all values based on the threshold
            for index in indexs:
        ...
        for index in range(4): #30
            if index < 1: #10
                ...
                top10S2BioMarkers.append(list(proteinDict.keys())[list(proteinDict.values()).index(top10S2[index])])
            if index < 3: #15
            ...
            top30S2BioMarkers.append(list(proteinDict.keys())[list(proteinDict.values()).index(top30S2[index])])
        ...
        top10DisjResultsSample = decisionTreeSample(top10DisjunctiveBioMarkers, "Fold " + str(counter) + " ~ Decision Tree ~ Top 10 Disj w/ S")
        top15DisjResultsSample = decisionTreeSample(top15DisjunctiveBioMarkers, "Fold " + str(counter) + " ~ Decision Tree ~ Top 15 Disj w/ S")
        top30DisjResultsSample = decisionTreeSample(top30DisjunctiveBioMarkers, "Fold " + str(counter) + " ~ Decision Tree ~ Top 30 Disj w/ S")
    ...
    def makeROC():
        ...
        with open('predictProbaScores' + str(method) + str(threshold) + '_75.csv','w') as csvfile:
...
def printing():
    with open('resultsGastricCancer'+ str(method) +str(threshold)+'_75.csv', 'w') as csvfile:
        filewriter = cs.writer(csvfile, delimiter=',',
            quotechar='|', quoting=cs.QUOTE_MINIMAL)
        filewriter.writerows(finalPrintData)
        print("Data printing done")

    with open('./biomarkers' + str(method) + str(threshold) + '_75.csv', 'w') as csvfile:
                ROCwriter = cs.writer(csvfile, delimiter=',',
