...
#inputs data of all Protein Names
...
#variables to alter hyperparameters of GBT
learningrate = 0.25
maxDepth = 2
thresholdInput = 0.6
method = "FINAL_MLP_UVCausal_All_Biomarkers_" + str(thresholdInput)[0] + "_" + str(thresholdInput)[2]
...
for threshold in varyingThresholds:
    ...
    tempString = ""
    #baseline Approach is a GBT which uses all biomarkers wihtout any feature selection
    ...
    #Causal Approach is a GBT which uses biomarkesr limited down by casual metric
    tempString = "23"
    UVData = dataList.copy()
    for index1 in range(len(UVData)):
        for value in range(len(UVData[0])):
            point = float(UVData[index1][value])
            if value > 1:
                if point > threshold:
                    UVData[index1][value] = 1
                else:
                    UVData[index1][value] = 0
    def combinedCausalityApproach(indexs, trainTotalData,counter,testData,testIndex):
        ...
        """
        featureSelectionData = UVData.copy()
        featureSelectionData.pop(testIndex[0])
        df = pd.DataFrame(featureSelectionData)
        xfeatures = df.iloc[:,2:]
        yFeatures = df[1]
        xfeatures.columns = proteinList
        bestfeatures = SelectKBest(score_func=chi2, k=30)
        fit = bestfeatures.fit(xfeatures,yFeatures)
        dfscores = pd.DataFrame(fit.scores_)
        dfcolumns = pd.DataFrame(xfeatures.columns)
        featureScores = pd.concat([dfcolumns,dfscores],axis=1)
        dfscores.dropna()
        avgFeatureScore = 0
        for point in dfscores.values:
            if "." in str(point[0]):
                newscore = avgFeatureScore + point[0]
                if newscore == np.nan:
                    avgFeatureScore = avgFeatureScore
                else:
                    avgFeatureScore += point[0]

        a = len(dfscores.values)
        avgFeatureScore = avgFeatureScore / len(dfscores.values)
        ...
        """
        usableProteins = []
        #creates list of proteins with univariate measure > avg univariate measure
        for protein in featureScores.values:
            if protein[1] > avgFeatureScore:
                usableProteins.append(protein[0])
        ...
        for bioMarker in usableProteins:
            ...
            #adds all samples that are positive for each biomarker to their own data sets (D_r_)
            for index in indexs:
        ...
        data = [["Biomarker","Causal Value"]]
        for marker in list(DisjunctiveEAVG.keys()):
            data.append([marker, DisjunctiveEAVG[marker]])
        with open('causalValues.csv', 'w') as csvfile:
            filewriter = cs.writer(csvfile, delimiter=',',
                quotechar='|', quoting=cs.QUOTE_MINIMAL)
            filewriter.writerows(data)
            print("Data printing done")
        #picks the top 1,3, and 4 biomarkers based on causal metric
        top10Disjunctive = DisjunctiveEAVGList[-10:]
        top15Disjunctive = DisjunctiveEAVGList[-20:]
        top30Disjunctive = DisjunctiveEAVGList[-30:]
        """top10Disjunctive = DisjunctiveEAVGList[-1:]
        top15Disjunctive = DisjunctiveEAVGList[-3:]
        top30Disjunctive = DisjunctiveEAVGList[-4:]"""
        ...
        #created List shown above with top bioMarkers depending on Conjunctive/Disjunctive/S2 - - - List contains names of BioMarkers 
        ...
        #trains the model using an RF Model, as there is no sample data for gastric dataset
        def decisionTreeSample(bioMarkerList, type):
            ...
            #creates the RF based on hyperparameters and trains the tree
            ...
            #binarizes the data for the test fold
            clfTestData = [(False, [])]
            clfTestData.pop(0)
            ...
            #finds the probability values used for ROC Curves 
            algoResults = [()]
            algoResults.pop(0)
            ...
            #stores results of the model based on if it was a case or control
            ...
            #assigns recall or spec a -1 to allow for calculations later on
            if recallDenom == 0:
                recallDenom = 1
                recallNum = -1
            if precisionDenom == 0:
            ...
            #final calculation of sens and spec for this fold
            recall = recallNum/recallDenom
            precision = precisionNum/precisionDenom
            specificity = specNum/specDenom
        ...
        #trains the model using only biomarkers data and a GBT Model based on Hyperparameters (comments are exact same as method decisionTreeSample, but now we train with a GBT instead of RF)
    ...
    for set in folds[0:25]:
        ...
        #creates an array containing values in format [(case or control),[all biomarker values binarized]] for train set
        baselineTrain = []
        baselineAffected = []
        for x in set[0]:
            if (dataList[x][1] == "0"):
        ...
        #creates an array containing values in format [(case or control),[all biomarker values binarized]] for test set
        for x in set[1]:
            if (dataList[x][1] == '0'):
        ...
        #creates the GBT for the baseline appraoch 
        ...
        #runs and stores values from baseline appraoch
        ...
        #runs and stores values from casual approach
        ...
        #used to find ROC curves in previous methods, not used anymore
        typertwo = 0
        if typertwo == 10:
            fig, ax = plt.subplots()
    ...
    #generates and saves a csv file containing predictProba values for ROC Curves
    def makeROC():
        ...
        with open('predictProbaScores' + str(method) + str(threshold) + '_25.csv','w') as csvfile:
    ...
    #does final computation for results (sens, spec, precision)
    def finalComputation():
        ...
        #takes out all -1 values assigned before
...
#prints the results of the program and biomarkers in each fold
def printing():
    with open('resultsGastricCancer'+ str(method) +str(threshold)+'_25.csv', 'w') as csvfile:
        filewriter = cs.writer(csvfile, delimiter=',',
            quotechar='|', quoting=cs.QUOTE_MINIMAL)
        filewriter.writerows(finalPrintData)
        print("Data printing done")

    with open('./biomarkers' + str(method) + str(threshold) + '_25.csv', 'w') as csvfile:
                ROCwriter = cs.writer(csvfile, delimiter=',',
