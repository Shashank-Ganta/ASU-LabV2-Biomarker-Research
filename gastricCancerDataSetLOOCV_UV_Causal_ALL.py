...
"""with open ('./gastricDataSetIgA.csv') as csv_file:
    ...
    for row in csv:
        ...
        if proteinName != '' and proteinName != 'null':
            proteinName += "_IgA"
            proteinDict[proteinName] = []
            proteinList.append(proteinName)
    proteinDict.pop("Protein name_IgA")
    proteinList.pop(0)
with open ('./gastricDataSetIgG.csv') as csv_file:
    csv = cs.reader(csv_file, delimiter=',')
    for row in csv:
        proteinName = str(row[0])
        if proteinName != '' and proteinName != 'null' and 'Protein name' not in proteinName:
            proteinName += "_IgG"
            proteinDict[proteinName] = []
            proteinList.append(proteinName)
    #proteinDict.pop("Protein name_IgG")
    #proteinList.pop(0)"""
...
maxDepth = 2
thresholdInput = 1.0
method = "CausalScores" + str(thresholdInput)[0] + "_" + str(thresholdInput)[2]
...
for threshold in varyingThresholds:
    ...
    def combinedCausalityApproach(indexs, trainTotalData,counter,testData,testIndex):
                ...
                for SecondaryProtein in usableProteins:
                    ...
                    if (Similiarity(Protein, SecondaryProtein) and Protein != SecondaryProtein):
                        ...
                        for index in range(len(ProteinCalculateData)):
                            ...
                            tempDisjunctiveR_r_.append((sampleType, float(ProteinCalculateData[index][0]) == 1 or float(SecondaryProteinCalculateData[index][0]) == 1))
                            tempDisjunctiveNotR_r_.append((sampleType,float(ProteinCalculateData[index][0]) == 0 or float(SecondaryProteinCalculateData[index][0]) == 1 ))
        ...
        def decisionTreeSample(bioMarkerList, type):
            ...
            #Training the XGB Model on Training Dataset
            clf = XGBClassifier(**params)
            clf = clf.fit(clfTrainData, clfAffectedData)
            ...
            """            dot_data = exp   ort_graphviz(clf.estimators_[0],filled=True, impurity=True, rounded=True) 
            ...
            algoResults = [()]
            algoResults.pop(0)
            #Testing Trained Model on Testing Data
    ...
    def finalComputation():
        ...
        #formatting 
        for t in range(1):
            #Computing Evalutation Metrics for Model 
...
def printing():
    ...
    with open('./causalScoresUVCausal' + str(threshold) + '.csv', 'w') as csvfile:
                ROCwriter = cs.writer(csvfile, delimiter=',',
                    quotechar='|', quoting=cs.QUOTE_MINIMAL,lineterminator = '\n')
                #filewriter.writerows(fieldName)
