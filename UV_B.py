...
rowIndex = 0
with open("C:\\Users\\shash\\VS Code Programs\\Python\\ResearchWork\\Gastric Cancer Data\\gastricSBATCHFiles\\gastricDataSetTotalFormattedReal.csv") as csv_file:
...
for threshold in varyingThresholds:
    ...
    def combinedCausalityApproach(indexs, trainTotalData,counter,testData,testIndex):
        ...
        """        for index1 in range((xfeatures.values.shape[0])):
            for value in range(xfeatures.values[index1].size):
                point = float(xfeatures.values[index1][value])
                if point > threshold:
                    xfeatures.values[index1][value] = 1
                else:
                    xfeatures.values[index1][value] = 0"""
        ...
        top30 = featureScores.nlargest(30,'Score')
        top30ProteinAlternateFeature = []
        for protein in top30.values:
            top30ProteinAlternateFeature.append(protein[0])

        top10DisjunctiveBioMarkers = top30ProteinAlternateFeature[:10]
        top15DisjunctiveBioMarkers = top30ProteinAlternateFeature[:15]
        top30DisjunctiveBioMarkers = top30ProteinAlternateFeature[:30]
        top1DisjunctiveBioMarkers = top30ProteinAlternateFeature[:1]
        top3DisjunctiveBioMarkers = top30ProteinAlternateFeature[:3]
        top4DisjunctiveBioMarkers = top30ProteinAlternateFeature[:4]
            ...
            for dataSet in trainTotalData:
                    ...
                    """            testIndexs = []
                            ...
                            for index in indexs:
                                ...
                                clfTrainData[counter].extend([isIgG,isIgA,is30_39,is81_,isM,isF])
                                counter+=1"""
    ...
    for set in folds:
        ...
        #clf = GradientBoostingClassifier(n_estimators=10, learning_rate=learningrate,max_depth=1, random_state=0)
        clf = LogisticRegression(solver='lbfgs')
        clf = clf.fit(trainData, affectedData)
    ...
    def makeROC():
        ...
        #'predictProbaScores' + str(method) + str(threshold) + '.csv'
        with open("C:\\Users\\shash\\VS Code Programs\\Python\\ResearchWork\\Gastric Cancer Data\\gastricSBATCHFiles\\predictProba_UV_14.csv",'w') as csvfile:
...
def printing():
    #'./resultsGastricCancer_LR_'+ str(method) +str(threshold)+'.csv'
    with open("C:\\Users\\shash\\VS Code Programs\\Python\\ResearchWork\\Gastric Cancer Data\\gastricSBATCHFiles\\resultsLR_UV_B.csv", 'w') as csvfile:
