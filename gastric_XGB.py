...
learningrate = 0.25
maxDepth = 2
method = "FINAL_XGB_UV_All_Biomarkers_1_4"

#creates arrays to store predictProba Values for ROC Curves
disj10Proba = []
disj15Proba = []
disj30Proba = []
disj10wSProba = []
...
#begins format for final printing into csv file
...
#dictates which threshold is used
varyingThresholds = [1.4]
for threshold in varyingThresholds:
    ...
    def combinedCausalityApproach(indexs, trainTotalData,counter,testData,testIndex):
        ...
        """
        featureSelectionData = dataList.copy()
        ...
        avgFeatureScore = np.average(dfscores)
    ...
    def makeROC():
        ...
        with open('predictProbaScores' + str(method) + str(threshold) + '.csv','w') as csvfile:
...
def printing():
    with open('resultsGastricCancer'+ str(method) +str(threshold)+'.csv', 'w') as csvfile:
        filewriter = cs.writer(csvfile, delimiter=',',
            quotechar='|', quoting=cs.QUOTE_MINIMAL)
        filewriter.writerows(finalPrintData)
        print("Data printing done")

    with open('./biomarkers' + str(method) + str(threshold) + '.csv', 'w') as csvfile:
                ROCwriter = cs.writer(csvfile, delimiter=',',
