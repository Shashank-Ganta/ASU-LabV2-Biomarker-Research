...
for threshold in varyingThresholds:
    ...
    def makeROC():
        ...
        with open('predictProbaScores' + str(method) + str(threshold) + '_100.csv','w') as csvfile:
...
def printing():
    with open('resultsGastricCancer'+ str(method) +str(threshold)+'_100.csv', 'w') as csvfile:
        filewriter = cs.writer(csvfile, delimiter=',',
            quotechar='|', quoting=cs.QUOTE_MINIMAL)
        filewriter.writerows(finalPrintData)
        print("Data printing done")

    with open('./biomarkers' + str(method) + str(threshold) + '_100.csv', 'w') as csvfile:
                ROCwriter = cs.writer(csvfile, delimiter=',',
